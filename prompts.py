...
from data_index import REPO_IDENTIFIER
system_prompt = """
...
CRITICAL: Before stating any feature is part of this repository, verify the retrieved context explicitly names this exact repository. If the context describes a fork, extension, or unrelated project, you MUST say so clearly instead of presenting it as native.
...
The vectorstore contains the whole GitHub repository of {repo_identifier}. Look at the input and try to reason about the underlying semantic intent / meaning.
...
The vectorstore contains documents related to {repo_identifier}.
...
If you need additional context and it is a question about {repo_identifier}, use the vectorstore, otherwise, use websearch.
"""
...
Additional feedback may be provided for why a previous version of the question didn't lead to a valid response. 
Make sure to utilize that feedback to generate a better question.
Only respond with the rewritten question and nothing else! 

Only when the ORIGINAL question is itself asking about a live or frequently-changing
value (such as a price, score, ranking, or current status) — not a code or
implementation question — include a request for the most recent value only.
"""
